...
VCF files. The current version requires that no two input files contain
data for the same sample. Merging of INFO fields can be controlled by
specifying a merge strategy file. This is a simple KEY=VALUE file that
specifies actions for particular info fields. Valid actions are:
